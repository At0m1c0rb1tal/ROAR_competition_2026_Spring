...
class RoarCompetitionRule:
    ...
    def initialize_race(self):
        ...
        self.waypoint_logs = []
        self._start_time = self.world.last_tick_elapsed_seconds
        self._last_logged_elapsed = 0.0
    ...
    async def tick(
        ...
        if self.furthest_waypoints_index > previous_furthest_index:
            elapsed = self.world.last_tick_elapsed_seconds - self._start_time
            delta_elapsed = elapsed - self._last_logged_elapsed
            speed_kmh = float(np.linalg.norm(self.vehicle.get_linear_3d_velocity()) * 3.6)
            for wp_idx in range(previous_furthest_index + 1, self.furthest_waypoints_index + 1):
                waypoint = self.waypoints[wp_idx]
                row = {
                    "waypoint_index": int(wp_idx),
                    "elapsed_seconds": float(elapsed),
                    "delta_seconds": float(delta_elapsed),
                    "speed_kmh": float(speed_kmh),
                    "x": float(waypoint.location[0]),
                    "y": float(waypoint.location[1]),
                    "z": float(waypoint.location[2]),
                }
                self.waypoint_logs.append(row)
                # print(
                #     f"reach waypoint {wp_idx} "
                #     f"elapsed={elapsed:.2f}s delta={delta_elapsed:.2f}s "
                #     f"speed={speed_kmh:.2f} at {waypoint.location}"
                # )
    ...
    def write_waypoint_log(self, output_path: Path):
...
async def evaluate_solution(
    ...
    log_path = Path(__file__).resolve().parent / "debugData" / "waypoint_speed_log.csv"
    rule.write_waypoint_log(log_path)
    print(f"Waypoint log written to {log_path}")
    vehicle.close()
    if enable_visualization:
        viewer.close()
