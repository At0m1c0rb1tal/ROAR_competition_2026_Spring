...
class ThrottleController:
    ...
    def run(
        ...
        if current_section in [12, 13, 14, 15]:
        ...
        if current_section in [12, 13, 14, 15]:
            if 0 < self.brake_ticks and self.brake_ticks < 5 and speed_excess < 8 and br_count > 5:
                throttle = 0.2
        elif current_section in [16, 17, 18]:
            ...
            if 0 < self.brake_ticks and self.brake_ticks < 3 and speed_excess < 8 and br_count > 5:
                ...
                # throttle = prev_throttle + 0.03
                throttle = 0.3
        elif current_section in [30, 31, 32, 33, 34, 35] and current_speed < 150:
    ...
    def get_throttle_and_brake(
        ...
        if current_speed > 100:
            ...
            if current_section not in [30, 31, 32, 33, 34, 35]:
                r4 = self.get_radius(
                    [
    ...
    def speed_for_turn(
        ...
        effective_distance = self._scaled_brake_distance(distance)
        d = (1 / 675) * (target_speed**2) + effective_distance
        max_speed = math.sqrt(825 * d)
        return SpeedData(effective_distance, current_speed, target_speed, max_speed, name, r)

    def _scaled_brake_distance(self, distance: float) -> float:
        scale = max(0.25, float(getattr(self, "brake_distance_scale", 1.0)))
        # Higher scale means the controller behaves as if the turn is closer,
        # so it starts braking earlier. Lower scale delays braking.
    ...
    def get_target_speed(self, radius: float, current_section: int):
        ...
        if current_section in [9, 10, 11]:
            mu = 3.35
        if current_section in [12, 13, 14, 15]:
            mu = 3.3
        if current_section in [16, 17, 18]:
            mu = 2.85
        if current_section in [23, 24]:
            mu = 3.3
        if current_section in [30, 31, 32, 33, 34, 35]:
            mu = 2.1
    ...
    def speed_for_turn_new(
        ...
        # Takes in a target speed and distance and produces a speed that the car should target. Returns a SpeedData object
        distance = self._scaled_brake_distance(distance)
        if distance <= 0:
